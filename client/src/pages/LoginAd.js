...
export default function LoginAd() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setModalMessage(err?.response?.data?.message || "Invalid admin credentials.");
      setModalOpen(true);
    } finally {
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    ...
    const material = new THREE.PointsMaterial({
      color: 0xff4500, // Red/Orange for Admin
      size: 1.5,
      map: getTexture(),
      transparent: true,
      opacity: 0.8,
      ...
      <div className="flex items-center justify-center w-full h-screen relative z-10 px-4">
        <div className="w-full max-w-[420px] animate-fade-in-up">
          ...
          <div className="bg-gray-900/60 backdrop-blur-xl rounded-3xl shadow-2xl p-8 sm:p-10 border border-white/10 relative overflow-hidden">
            {/* Gradient Top Bar */}
            <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-red-500 via-orange-500 to-red-600" />

            <div className="text-center mb-8">
              <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-gradient-to-br from-red-500 to-orange-600 shadow-lg shadow-orange-500/20 mb-6 transform rotate-3 hover:rotate-6 transition-transform">
